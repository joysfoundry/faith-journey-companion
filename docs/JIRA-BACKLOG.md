...
| [ACTS-143](../stories/ACTS-143.md) |  | P3 | Story | In Progress | **About the app — menu entry (vision, beta purpose, "everything is local")** | _New (JC)._ Add an **About** entry to the menu ([`nav-links.ts`](../src/components/layout/nav-links.ts) secondary; side rail + mobile drawer) → `/about` route or dialog: what the app is, the vision (from [`docs/ACTS-PRD.md`](ACTS-PRD.md)), why it's a beta, and that **all data stays local** on the device (no email/account/server today — ties to ACTS-82). Warm, brief copy; link to Settings "Start over". Relates ACTS-90/82. |
