...
| ACTS-116 |  | P4 | Story | To Do | **Project status dashboard / daily-weekly brief** | _New (JC idea from the gap review — "GREAT info for a dashboard, especially daily/weekly brief")._ A living status view (built/partial/none + open stories + recent commits) rendered as a brief. Explore the shape; relates to the `/prd-sync` inventory (ACTS-104/105). |
| [ACTS-117](../stories/ACTS-117.md) |  | P2 | Story | In Progress | **Rebase ACTS-PRD onto v8 + apply reconcile checklist** | _New._ Make **v8** the content base of `docs/ACTS-PRD.md` (single canonical `.md`, bump to v3); re-apply Mission + two-part flow + Solution-Idea + "What's shipped" inventory + shipped-notes; apply [`V8-CODE-GAP.md`](V8-CODE-GAP.md) reconcile notes; add shipped-but-absent sections (Share, Lectio, Bible settings, Mystery bodies, Song); re-export .docx. **Next chat starts here.** Handoff [session-01](../stories/ACTS-117/session-01.md). Relates ACTS-104. |
