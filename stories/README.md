...
|----|------|-------|--------|-----------|-----------|---------|
| [ACTS-133](ACTS-133.md) |  | Daily Rosary — launch an external app (Hallow) instead of the in-app session | In Progress | — | ✅ Built + browser-verified (`tsc` clean). `prayer/apps.ts` (Hallow universal/app link `collections/16/`), `AppSettings.daily_rosary_*`, Settings mode toggle, external launch on Home card + Plan pinned row. Decision A (single smart URL) + auto-log as prayed (idempotent completed session; `session.external_app`; detail shows "Prayed in Hallow"). Next: `/save` | 2026-09-02 |
