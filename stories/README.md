...
| [ACTS-107](ACTS-107.md) |  | Litany of the Departed devotion (Rosary + Litany structure) | To Do | ACTS-106 | **READY TO START (all blocks done).** Assemble via ACTS-110 blocks: `tpl-rosary-for-the-dead` (119) → litany block `tpl-litany-faithful-departed`/`tpl-litany-loreto` (120) → Closing block (`collect-fidelium`+`collect-departed-kindred`) → `eternal-rest` → `sign-of-the-cross`; dedicate via `for_whom` (121); bump STORAGE_KEY v33→v34; browser-verify + share. Full assembly brief in the pointer | 2026-08-30 |
