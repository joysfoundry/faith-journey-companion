...
#   scripts/published.sh --md              # markdown snapshot for the backlog ledger
...
for id in $STORY_IDS; do
  ...
  if [ "$total" -eq 0 ]; then
    ...
    MD_FLAG+="- **${id}** (${st}) — deployed to prod, not released${flag:+ (flag: \`$flag\`)}"$'\n'
  else
    n_rel=$((n_rel+1))
...
# --- markdown snapshot (for the backlog ledger) -----------------------------
if [ "$MD" -eq 1 ]; then
  echo "<!-- RELEASE-STATUS:START — generated by \`scripts/published.sh --md\`; do not hand-edit -->"
  echo "## Release status"
  echo
  echo "_Where each story sits on the path to users — **generated**, not hand-maintained;"
  echo "regenerate with \`scripts/published.sh --md\`. Snapshot: **$(date +%Y-%m-%d)**."
  echo "Production line: \`${REF}\` @ \`${REF_SHA}\`. Released = shipped via Lovable **PUBLISH**;"
  echo "deploy ≠ release (code behind a flag is in production but not released to users)._"
  echo
  echo "**Not yet Published (${n_unrel})** — on the path, not live to users:"
  echo
  [ -n "$MD_UNREL" ] && printf '%s' "$MD_UNREL" || echo "- _none_"
  echo
  echo "**In production, behind a flag (${n_flag})** — deployed, not released to users:"
  echo
  [ -n "$MD_FLAG" ] && printf '%s' "$MD_FLAG" || echo "- _none_"
  echo
  echo "_All other numbered stories with commits are **Published to production** (${n_rel})."
  echo "Stories with no commits yet (${n_none}) are omitted._"
  echo "<!-- RELEASE-STATUS:END -->"
  exit 0
fi
