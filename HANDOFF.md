...
- ⚠️ **BUMP `STORAGE_KEY` v7 → v8 (REQUIRED for the USCCB seed to show).** `src/lib/prayer/store.ts:39`. The seed changed but the key didn't, and persisted localStorage overrides the seed's `prayers` array (`store.ts:150` `{ ...seed, ...parsed }`), so the 13 new prayers are **invisible** on any browser holding a v7 DB. This is a one-line code change → do it via `/save` (not committed in the handoff). After bumping, verify the new prayers render in the library and pick one to pray.
