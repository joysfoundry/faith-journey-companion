...
- ✅ **`STORAGE_KEY` bumped v7 → v8** (`36a8596`) so the USCCB seed prayers reload past persisted localStorage. **Still to do:** verify in-browser that the 13 new prayers render in the library and pray one (not yet done this session — another chat's dev server was occupying the folder).
